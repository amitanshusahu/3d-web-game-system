...
import { getSfx } from '../../World/sfx/sfxRegistry'
...
export function MobileHome(props: JSX.IntrinsicElements['group']) {
  ...
  useEffect(() => {
    ...
    if (!action) return
    // Half-speed so the house's mechanical unfolding reads slowly.
    action.reset().fadeIn(0.5).setEffectiveTimeScale(0.5).play()

    // Gears ambience rides the animation: it starts exactly when the house
    // begins to move and is torn down with the animation.
    const gears = new Howl({ src: [getSfx('props')[0]], loop: true, volume: 0.5, preload: true })
    gears.play()
